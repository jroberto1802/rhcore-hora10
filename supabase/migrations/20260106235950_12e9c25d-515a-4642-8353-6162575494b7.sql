-- Dropar a função antiga e recriar com a mesma assinatura original
DROP FUNCTION IF EXISTS public.funcionarios_safe();

-- Recriar a função funcionarios_safe() mantendo a assinatura original
CREATE OR REPLACE FUNCTION public.funcionarios_safe()
 RETURNS TABLE(id uuid, empresa_id uuid, codigo text, nome_completo text, nome_abreviado text, cargo_atual text, tipo_cargo text, setor_atual text, tipo_contrato text, data_admissao date, data_demissao date, foto_url text, pcd boolean, created_at timestamp with time zone, updated_at timestamp with time zone, cpf text, rg text, ctps text, pis text, serie text, telefone text, email text, endereco text, numero_endereco text, bairro text, cidade text, uf text, cep text, salario_atual numeric, banco text, agencia text, numero_conta text, tipo_conta text, chave_pix text, telefone_emergencia text, nome_contato_emergencia text, tipo_sanguineo text, genero text, data_nascimento date)
 LANGUAGE sql
...
AS $function$
  SELECT 
    ...
    -- Campos sensíveis: retorna dados reais se usuário tem permissão, NULL caso contrário
    CASE WHEN public.can_view_sensitive_data(auth.uid(), f.empresa_id) 
